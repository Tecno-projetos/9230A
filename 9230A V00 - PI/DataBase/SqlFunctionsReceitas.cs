...
namespace _9230A_V00___PI.DataBase
{
    class SqlFunctionsReceitas
    {
        ...
        private static void Create_Table_Receita(string tableName)
        {
            if (Utilidades.VariaveisGlobais.DB_Connected_GS)
            {
                try
        ...
        public static int IntoDate_Table_Produtos(string tableName, string nomeReceita, float pesoBase, int codigoProduto, float pesoPorProduto, string observacao)
        {
            int ret = -1;
            ...
            if (Utilidades.VariaveisGlobais.DB_Connected_GS)
            {
                try
                {
                    ...
                    string query = "INSERT into '"+ tableName + "' (NomeReceita, PesoBase, CodigoProduto, PesoPorProduto, Observacao) VALUES (@NomeReceita, @PesoBase, @CodigoProduto, @PesoPorProduto, @Observacao)";
